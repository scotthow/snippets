# Beginning of the new measurement year where we don't yet have reported rates
    ...
    # Find the max existing cycle_id to determine what we should check for
    if len(df) > 0:
        max_cycle_id = df['cycle_id'].max()
        # Extract the month from max_cycle_id
        max_month = int(max_cycle_id.split('-')[1])
    else:
        # If no data exists yet, set max_month to 1
        max_month = 1
    
    # Create possible cycle_ids up to the current max
    all_possible_cycle_ids = [f"{current_year}-{str(i).zfill(2)}" for i in range(1, max_month + 1)]
    ...
    for missing_id in missing_cycle_ids:
        ...
        if month_num == 1:
            ...
            if len(df) > 0:
                ...
                # This assumes you have numeric columns to zero out - adjust as needed
                ...
                row_to_copy = pd.Series()
            
            # Update cycle_id and set as synthetic
