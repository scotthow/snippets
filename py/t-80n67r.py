...
# Update forecasting list
# If the forecast is output/forecast then remove it from the list
FC_LIST = Config.FC_LIST
for population, measure, model_id in FC_LIST:
    # Serch for the forecast file; population, measure, model id will be in the name
    forecast_file = f'output/forecasts/results_{population}_{measure}_{model_id}_prod_{TODAY}.csv'
    if os.path.exists(forecast_file):
        print(f"Removing {forecast_file} from the forecast list")
        FC_LIST.remove((population, measure, model_id))
if len(FC_LIST) == 0:
    print("All forecasts have been completed!")
    sys.exit(0)
