...
def update_forecast_list(fc_list: List[Tuple[str, str, str]], today: str) -> List[Tuple[str, str, str]]:
    """
    Update forecast list by removing already processed forecasts.
    
    Args:
        fc_list: List of tuples containing (population, measure, model_id)
        today: Current date string
        
    Returns:
        Updated forecast list
    """
    logging.info(f"Initial forecast list size: {len(fc_list)}")
    
    # Create new list excluding existing forecasts
    updated_list = [
        (population, measure, model_id) 
    ...
    # Log removed forecasts
    removed = set(fc_list) - set(updated_list)
    for item in removed:
        logging.info(f"Removing forecast for: {item}")
    
    logging.info(f"Remaining forecasts to process: {len(updated_list)}")
    
    if not updated_list:
        logging.info("All forecasts have been completed!")
        sys.exit(0)
...
# First update the forecast list
try:
    FC_LIST = update_forecast_list(FC_LIST, TODAY)
except Exception as e:
    logging.error(f"Error updating forecast list: {str(e)}")
    raise
...
def main():
    ...
    for population, measure, model_id in FC_LIST:
        ...
        df.to_excel(f'output/qa/df_{population}_{measure}_pre_synthetic_adj.xlsx', index=False)
        ...
        # Save to CSV - for testing
        df.to_excel(f'output/qa/df_{population}_{measure}_post_inverse_adj.xlsx', index=False)
        ...
        df.to_excel(f'output/qa/df_{population}_{measure}_post_synthetic_adj.xlsx', index=False)
        ...
        last_year_rate = df[df['cycle_id'] == last_year_cycle_id]['measure_rate'].values[0]
        ...
        fc_cols = [
            'measurement_year', 'book_name', 'measure_id_key', 'measure_rate', 'cycle_id'
        ]
        df = df[fc_cols]
        
        # Create forecaster instances and generate predictions
        if model_id in ['hw-damped', 'sarima', 'custom-mirror']:
            ...
            results_df_prod['curr_cycle_id'] = curr_cycle_id
            results_df_prod['curr_rate'] = curr_rate

            results_df_prod.to_csv(f'output/forecasts/results_{population}_{measure}_{model_id}_prod_{TODAY}.csv', index=True)
            results_df_prod.to_excel(f'output/qa/results_{population}_{measure}_{model_id}_prod_{TODAY}.xlsx', index=True) # for QA
    ...
    dir_path = 'output/forecasts'
    files = os.listdir(dir_path)
    for file in files:
        if file.endswith('.csv'):
            df = pd.read_csv(f'{dir_path}/{file}')
            df_forecast_combined = pd.concat([df_forecast_combined, df], ignore_index=True)
            # Save to CSV
            # df_forecast_combined.to_csv(f'output/FINAL_{TODAY}.csv', index=False)
