...
class HEDISPreprocessor:
    ...
    def create_geographic_features(self, df):
        ...
        # State encoding
        df_copy['state_encoded'] = self.state_encoder.fit_transform(df_copy['state'])
    ...
    def create_sdoh_composite_scores(self, df):
        """
        Create composite Social Determinants of Health scores.
        """
        df_copy = df.copy()
...
class FeatureSelector:
    ...
    def _univariate_selection(self, X, y):
        ...
        combined_scores = (f_scores / f_scores.max() + mi_scores / mi_scores.max()) / 2
...
if __name__ == "__main__":
    ...
    print("Testing XGBoost Pipeline with SMOTE")
    print("-" * 50)
    ...
    print("\nTesting Logistic Regression Pipeline with SMOTE")
    print("-" * 50)
    ...
    visualizer.plot_classification_report(y_test, y_pred_xgb, 'XGBoost',
                                         save_path='hedis_classification_xgboost.png')
    
    # 3. Classification Report - Logistic Regression
    visualizer.plot_classification_report(y_test, y_pred_lr, 'Logistic Regression',
                                         save_path='hedis_classification_lr.png')
    ...
    visualizer.plot_roc_curves(models_data, save_path='hedis_roc_curves.png')
    
    # 5. Feature Importance
    visualizer.plot_feature_importance(xgb_pipeline.get_feature_importance(),
                                      top_n=15, save_path='hedis_feature_importance.png')
