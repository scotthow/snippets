...
if __name__ == "__main__":
    ...
    col_name = 'poverty_score'  # Change this to any float column name
    
    # Create analyzer
    analyzer = DistributionAnalyzer(df, col_name)
    ...
    text_filename = f"{col_name}_analysis_{timestamp}.txt"
    
    with open(text_filename, 'w') as f:
        sys.stdout = f
        
        # Run complete analysis
        print(f"Distribution Analysis Report")
        print(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("=" * 60)
        
        # Perform analysis with all tests
        results = analyzer.analyze(print_results=True, plot=False, 
                                 detect_outliers=True, test_normality=True)
    ...
    image_filename = f"{col_name}_distribution_{timestamp}.png"
    fig = analyzer.plot_distribution(figsize=(14, 10))
