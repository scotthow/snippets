...
class HEDISPreprocessor:
    ...
    def create_geographic_features(self, df):
        ...
        if self._state_mapping is not None:
            df_copy['state_encoded'] = df_copy['state'].map(self._state_mapping).fillna(0).astype(int)
        else:
...
if __name__ == "__main__":
    ...
    data = {
        ...
        "state": np.random.choice(["VA", "NC", "MD", "WV"], n), "zip_code": np.random.randint(20000, 99999, n),
        "latitude": np.random.uniform(35, 40, n), "longitude": np.random.uniform(-85, -75, n),
