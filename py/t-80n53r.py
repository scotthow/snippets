...
class EnhancedModelTrainer:
    ...
    def plot_enhanced_results(self, y_test, feature_importances):
        """Create enhanced visualizations focusing on minority class performance"""
        
        # 1. Feature Importance Chart (NEW)
        plt.figure(figsize=(12, 10))
        bars = plt.barh(feature_importances.index, feature_importances.values, color='#4ECDC4', alpha=0.8)
        plt.xlabel('Combined Importance Score', fontsize=12)
        plt.ylabel('Features', fontsize=12)
        plt.title('Top Selected Feature Importances', fontsize=16, fontweight='bold', pad=20)
        plt.grid(axis='x', linestyle='--', alpha=0.6)
        
        # Add value labels to the bars for clarity
        for bar in bars:
            width = bar.get_width()
            plt.text(width + 0.005, bar.get_y() + bar.get_height()/2.0, f'{width:.2f}',
                     va='center', ha='left', fontsize=9)

        plt.tight_layout()
        plt.show()
        
        # 2. Class Distribution Pie Charts (Before and After SMOTE)
        ...
        # 3. ROC-AUC Curves for All Models
        ...
        # 4. Minority Class Performance Comparison
...
def main_improved():
    ...
    compliance_score = (
        ...
        np.random.uniform(-20, 20, len(df))  # Add noise
    )
    ...
    # Create a pandas Series for feature importances for easier plotting
    all_scores_dict = dict(zip(preprocessor.feature_names, feature_scores))
    selected_scores = {name: all_scores_dict[name] for name in preprocessor.selected_features}
    feature_importances = pd.Series(selected_scores).sort_values()
    ...
    # Plot enhanced results, including the new feature importance chart
    trainer.plot_enhanced_results(y_test, feature_importances)
